...
class CurriculumExtractor:
    ...
    def _extract_from_headings(self, text: str) -> List[Dict[str, str]]:
        ...
        # Match complete titles until newline or sentence end
        section_pattern = r'(?:^|\n)(\d+)\.\s+([A-Z][^\n]+?)(?:\n|$)'
        ...
        for match in matches:
            ...
            # Clean up title - remove trailing punctuation and page numbers
            section_title = re.sub(r'\s+\d+\s*$', '', section_title)  # Remove trailing page numbers
            section_title = re.sub(r'\s*\.\s*$', '', section_title)  # Remove trailing period
            
            # Filter out likely non-headings
            # - Must be reasonable length (not too short, not too long)
            # - Should not contain excessive numbers (likely page refs)
            # - Should not be all numbers
            if (10 <= len(section_title) <= 150 and  # Reasonable length
                not re.search(r'\d{3,}', section_title) and  # No long number sequences
                not section_title.replace(' ', '').isdigit()):  # Not all numbers
    ...
    def _extract_from_chunks(self, chunks: List[str]) -> List[Dict[str, str]]:
        ...
        for i, chunk in enumerate(chunks):
            ...
            if (len(first_line) > 10 and len(first_line) < 150 and
                first_line[0].isupper() and
                not first_line.endswith('.') and
                len(first_line.split()) >= 2):
                
                # Clean up the title
                cleaned_title = first_line.strip()
                
                # Remove trailing page numbers
                cleaned_title = re.sub(r'\s+\d+\s*$', '', cleaned_title)
                
                # Only add if it's a complete sentence/phrase (ends with proper word)
                if cleaned_title and not cleaned_title.endswith(('the', 'a', 'an', 'of', 'for', 'in', 'on', 'at')):
                    curriculum.append({
                        'title': cleaned_title,
                        'type': 'section',
                        'order': i + 1
